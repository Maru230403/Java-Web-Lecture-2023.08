...
-- 고객별로 주문한  도서의 총 수량과 총 판매액
 ...
 /* HAVING */
 -- 책을 2권이상 출간한 출판사의 평균 책 가격 (조건문을 사용할때 group by를 사용할 때는 HAVING 사용) 
 select publisher, count(*), avg(price) from book
    group by publisher
    having count(*) >= 2;
--가격이 8000원 이상인 도서를 구매한 고객에 대하여 고객별 주문 도서의 총 수량 및 평균 금액
-- 단 ,두 권 이상 구매한 고객만 구한다.
select * from orders;
select custid, count(*), avg(saleprice) from orders where saleprice >= 8000
    group by custid
    having count(*) >= 2;
-- select 절에는 bookid 속성이 올수 없다 (왜????????)
-- 누가 책 매출액이 많은가?
...
select c.custid, c.name, c.address, c.phone, o.bookid, o.saleprice, o.orderdate
    ...
    order by c.custid;
-- 고객의 이름과 고객이 주문한 도서의 판매액을 검색하시오.
